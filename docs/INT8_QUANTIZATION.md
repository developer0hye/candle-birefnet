...
## Rust Dequantize Validation (Golden Tests)

Two golden tests verify that Rust dequantization produces identical output to Python:

```bash
cargo test golden_birefnet_int8 --release -- --nocapture
```

### Test 1: `golden_birefnet_int8_dequantized_inference`

Loads weights dequantized in **Python**, runs inference in **Rust**, compares against Python output.
This isolates the Rust inference engine — any difference means a model implementation bug.

### Test 2: `golden_birefnet_int8_rust_dequantize`

Loads the raw INT8 safetensors file, dequantizes in **Rust**, runs inference in **Rust**, compares against Python output.
This validates the full end-to-end Rust INT8 loading pipeline.

### Results

| Test | Max Error | Mean Error | Status |
|------|-----------|------------|--------|
| Python dequantize → Rust inference | 5.15e-5 | 3.35e-6 | ✓ |
| **Rust dequantize → Rust inference** | **5.15e-5** | **3.35e-6** | **✓** |

Both tests produce **identical output** (5.15e-5 max error), confirming:
- Rust dequantize formula matches Python exactly
- Per-channel broadcast, sign handling, and scale application are all correct
- No precision loss from the Rust dequantize implementation

Test data generated by `scripts/generate_golden_data_int8.py`.
...
| `scripts/generate_golden_data_int8.py` | Generate golden test data (Python dequantize + inference output) |
| `tests/golden.rs` | Rust golden tests including INT8 dequantize validation |
| `wasm/src/lib.rs` | WASM loader (FP32 only — INT8 dequantize done in JS worker) |
| `weights/birefnet_int8.safetensors` | Quantized weights |
